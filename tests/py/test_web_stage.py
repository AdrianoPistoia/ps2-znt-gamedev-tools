...
s = get("/api/stage?scene=s&step=3")          # hasta el say
...
assert ids["a"]["url"] and "ana.png" in ids["a"]["url"], ids["a"]     # sprite real
...
assert s["say"]["name"] == "Ana" and s["say"]["text"] == "Hola."

img = urllib.request.urlopen(B + "/api/asset?f=ana.png").read()       # assets
assert img[:8] == b"\x89PNG\r\n\x1a\n"
try:                                                                  # traversal bloqueado
    urllib.request.urlopen(B + "/api/asset?f=../../etc/passwd"); raise SystemExit("traversal NO bloqueado")
