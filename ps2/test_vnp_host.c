/* Checks vnp.c (the PS2 reader) against a blob generated by vniso.py, on the host. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "vnp.h"
...
int main(int argc, char **argv) {
    ...
    CHECK(d.n_scenes == 2);
    CHECK(d.n_chars == 1);
    CHECK(streq(&d, 0, "T"));                 /* title = string 0 */
    ...
    CHECK(s.img == VNP_NONE16);               /* no expression: base sprite */
    CHECK(vnp_step(&sc, &s) && s.op == OP_SAY && streq(&d, s.text, "Hello."));
    CHECK(vnp_step(&sc, &s) && s.op == OP_CHOICE && s.n_opts == 2 && s.opt_target[0] == 1 && s.opt_target[1] == 1);
    CHECK(vnp_step(&sc, &s) == 0);            /* end of scene 0 */

    CHECK(vnp_scene_begin(&d, 1, &sc) == 0);
    CHECK(vnp_step(&sc, &s) && s.op == OP_SAY && s.who == VNP_NONE16 && streq(&d, s.text, "bye"));
    ...
    /* audio and image: index (off, len) with the data after the header */
    CHECK(d.n_audio == 1);
    ...
    /* header only (what the ELF loads into RAM): everything still reads, the data stays on demand */
    { VnpDoc h; CHECK(vnp_open(&h, buf, d.head_size) == 0);
      ...
      VnpScene s2; VnpStep st; CHECK(vnp_scene_begin(&h, 1, &s2) == 0 && vnp_step(&s2, &st) && streq(&h, st.text, "bye")); }

    /* baked font: present and absent glyphs */
    CHECK(d.has_font && d.font_w == 8 && d.font_h == 16);
    CHECK(vnp_glyph(&d, 'H') != 0);
    CHECK(vnp_glyph(&d, 0x3000) == 0);       /* CJK: not in the atlas */
